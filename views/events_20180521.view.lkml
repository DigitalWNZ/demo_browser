...
explore: events_20180521 {
  hidden: no
  always_filter: {
    filters: {
      field: events_20180521.date_filter
      value: "7 days" #This is the default filter, can be overriden

    }

  }
  join: events_20180521__event_params {
    view_label: "Events 20180521: Event Params"
    sql: LEFT JOIN UNNEST(${events_20180521.event_params}) as events_20180521__event_params ;;
    relationship: one_to_many
  }

  join: events_20180521__user_properties {
    view_label: "Events 20180521: User Properties"
    sql: LEFT JOIN UNNEST(${events_20180521.user_properties}) as events_20180521__user_properties ;;
    relationship: one_to_many
  }
}

view: events_20180521 {
  sql_table_name: `allen-first.bingoblast.events_*`
    ;;
  filter: date_filter {
    hidden: no
    type: date
    ...
    sql:
      _TABLE_SUFFIX BETWEEN FORMAT_DATETIME("%Y%m%d",{%date_start date_filter %}) AND FORMAT_DATETIME("%Y%m%d",{% date_end date_filter %});;
    }
  ...
  measure: count {
    type: count
    drill_fields: [event_name, traffic_source__name, device__mobile_model_name, device__mobile_brand_name, device__mobile_marketing_name]
  }
}

view: events_20180521__event_params {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: events_20180521__user_properties {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
